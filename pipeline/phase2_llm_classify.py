...
_DEFAULT_MODEL: str = os.environ.get("GEMINI_MODEL", "gemini-3.6-flash")
...
def call_gemini_batch(
    ...
    Raises
    ------
    Transient errors are retried. Permanent API errors raise immediately so
    the caller cannot mistake an authentication, permission, or model failure
    for a valid classification containing no pages.
    """
    ...
    for attempt in range(1, _MAX_RETRIES + 1):
        try:
            ...
            if status == 429 or status >= 500:
                ...
                # Do not turn permanent API failures into an empty
                # classification. The downstream writer cannot distinguish
                # that from a legitimate all-XX response.
                raise RuntimeError(
                    f"Gemini API rejected the classification request (HTTP "
                    f"{status}) for model '{model}'. Check GEMINI_API_KEY "
                    f"permissions and GEMINI_MODEL. Response: {body_text}"
                ) from exc
...
def classify_page_batches(
    ...
    model:
        Gemini model to use (default: ``GEMINI_MODEL`` or
        ``gemini-3.6-flash``).
    inter_batch_delay:
