...
Stage 3 — Look-back density check only
    The auditor-signature and header filters are the primary selectors.
    Once a page survives Stage 1 and Stage 2, it is kept regardless of its
    numeric density. Density is only used as a secondary check for the two
    look-back pages immediately before each selected page. If a look-back page
    is numerically dense, it is included as an additional candidate; otherwise
    it is skipped. All page numbers are deduplicated across the selected pages
    and any qualifying look-back pages.
...
def extract_auditor_signatures(
    ...
    log.info("  Stage 3 - Look-back only: density is used only for the 2 preceding pages")
    ...
    #
    # For every Stage-1/2 survivor, include the page itself immediately (no
    # numeric-density gate on the main candidate). Then inspect the two pages
    # immediately before it (page_num-1 and page_num-2, clamped to page 1) and
    # apply the numeric-density rule only there. Any preceding page that passes
    # density is included as a look-back candidate; otherwise it is skipped.
    #
    # Preceding pages are included as plain page-number entries (no signature
    # block metadata) because they are only being admitted via the density-based
    # look-back rule.
    ...
    for candidate in stage12_survivors:
        ...
        # Primary selection is no longer gated by numeric density. Any page that
        # survives the auditor-signature and header filters is kept directly.
        density_pct = number_density(text) * 100
        log.info("Page %d digit density %.1f%% -> KEPT [primary selection, density ignored]", pn, density_pct)
